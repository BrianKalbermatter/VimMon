...
static void player_update(Entity *self, World *w, float dt) {
  ...
  if (r->key_down(KEY_LEFT) || r->key_down(KEY_A))
    self->x -= speed * dt;
  if (r->key_down(KEY_RIGHT) || r->key_down(KEY_D))
    self->x += speed * dt;
  if (r->key_down(KEY_UP) || r->key_down(KEY_W))
    self->y -= speed * dt;
  if (r->key_down(KEY_DOWN) || r->key_down(KEY_S))
    self->y += speed * dt;
