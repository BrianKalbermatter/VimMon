...
static int ollama_request(const char *user_prompt, char *out, int out_size) {
    ...
    snprintf(full_prompt, sizeof(full_prompt),
        ...
        "- Crear:     CUBO/ESFERA/PLANO/LUZ(nombre = <id>, posicion = (x,y,z), color = #hex)\n"
        "- Modificar: MOVER/ROTAR/ESCALAR/COLOR(nombre = <id>, ...)\n"
        "- Comportamiento: GIRAR(nombre = <id>, eje = y, velocidad = n)\n"
        ...
        "CUBO(nombre = caja, posicion = (1,0,0), color = #e63946, tamano = (1,1,1));\n"
        "GIRAR(nombre = caja, eje = y, velocidad = 0.5);\n\n"
        "Escena actual:\n%s\n\n"
        "Usuario: %s\n\nPAED delta:",
