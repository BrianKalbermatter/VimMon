...
- [x] **SECUENCIAS en PAED** (2026-08-12): `SECUENCIA DE <tipo>`, `SECUENCIA DE SALIDA` y `VENTANA DE <tipo>` en el AMBIENTE, y `ARR`/`AVZ`/`NFDS`/`FDS`/`CREAR`/`ESCRIBIR(sec,v)`/`CERRAR` ejecutando de verdad. Es la estructura que toman los parciales: medido sobre `apuntes/AED`, `SECUENCIA DE` aparece 41 veces y `avz(` 38, mientras que `ARREGLO[` **no aparece ninguna** — los arreglos entraron desde la wiki, no desde los examenes #fase2
- [x] El estado de las secuencias vive en `lang/src/secuencia.c`, aparte del interprete: el evaluador tambien lo necesita para `FDS`/`NFDS`, y ponerlo en `interpreter.c` habria hecho que `expr.c` dependiera del interprete que ya depende de el #fase2
- [x] De donde salen los datos de una secuencia lo decide el HOST (`interp_set_secuencia`), igual que `LEER`. Se piden ENTERAS y una sola vez al arrancar, no de a un elemento: la secuencia es un dato fijo del enunciado, y pedirla entera es lo que permite que `FDS` conteste sin adivinar si viene algo mas. La CLI los busca en un bloque `// - SECUENCIA <nombre>` del propio `.paed` #fase2
- [x] En una `SECUENCIA DE CARACTER` el tipo del elemento lo decide la DECLARACION y no el dato: el `5` de un legajo es el caracter `'5'` y no el numero 5. Es al reves que en `LEER`, que no tiene ninguna declaracion de donde agarrarse #fase2
- [x] Avanzar despues del fin de secuencia es ERROR, no un dato mas: el bucle corta con `NFDS`, asi que un `AVZ` de mas significa que el corte esta mal escrito. Devolver algo inventado dejaria el sintoma tres instrucciones mas abajo, sin relacion aparente con la causa #fase2
- [x] Varias instrucciones en una linea, separadas por `;` (`ARR(sec); AVZ(sec, v);`). El corpus las escribe asi cuando las dos son un solo gesto. El `;` sigue siendo TERMINADOR: lo que falta para los simulacros es poder OMITIRLO, que sigue abierto #fase2
